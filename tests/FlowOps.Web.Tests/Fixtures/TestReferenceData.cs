...
/// <summary>
/// The minimum reference data the Phase 5 pages need to be exercisable: one team, one category
/// belonging to it, memberships for the users whose access is being tested, and one existing
/// ticket. Test-only and scoped to the ephemeral Testcontainers database — this is not the demo
/// seeder (CLAUDE.md §14), which is Phase 15.
/// </summary>
/// <remarks>
...
internal static class TestReferenceData
{
    ...
    public static async Task<(int TeamId, int CategoryId, int TicketId)> SeedAsync(IServiceProvider services)
    {
        var db = services.GetRequiredService<FlowOpsDbContext>();
        var userManager = services.GetRequiredService<UserManager<ApplicationUser>>();

        var team = new Team(0, TeamName, DateTimeOffset.UtcNow);
        db.Add(team);
        ...
        var (ticketId, _) = await ticketService.CreateAsync(
            new CreateTicketRequest(
                ...
                ProjectId: null),
            new CurrentUser(agentId, UserRole.Agent, new HashSet<int> { team.Id }, new HashSet<int>()));

        return (team.Id, category.Id, ticketId);
    }
