...
**The interface is dark-only.** There is no light theme (see §11 — this may be a decision or an
omission; it is not yet recorded either way).
...
| 34px | 500 | −0.035em | 1 | Figures (tabular) |
...
- **Light theme.** Dark-only today. Decision or omission? Worth recording either way.
- **Motion.** Durations exist now (120–200ms `ease`, colour/background/border/transform only, no
  keyframes) but there's no `prefers-reduced-motion` guard yet, and no stated policy on whether one
  is needed given how little motion exists to reduce.
